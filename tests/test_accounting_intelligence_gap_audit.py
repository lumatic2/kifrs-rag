...
def test_gap_audit_covers_current_expansion_evidence() -> None:
    ...
    assert "decision queue, next-action summary, next-action sequence gate, reviewer invite action packet, invite send receipt, readiness index, external-action boundary, invite, response handling, after-send action matrix, outreach transition verifier, scheduled-session, RS3 notes-quality/capture-readiness/post-session final gate, operator execution brief, pre-send final gate, and close-state matrix are ready" in joined_gaps
