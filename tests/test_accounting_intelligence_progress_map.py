...
def test_progress_map_explains_current_position_and_results() -> None:
    ...
    assert progress["current_horizon"]["id"] == "real-accountant-session"
    assert progress["current_horizon"]["milestones"][0]["status"] == "completed"
    assert progress["current_horizon"]["milestones"][1]["status"] == "waiting_on_reviewer_invite"
    assert progress["next_leaf"] == "send_reviewer_invite"
