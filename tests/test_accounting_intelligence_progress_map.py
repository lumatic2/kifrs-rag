...
def test_progress_map_explains_current_position_and_results() -> None:
    ...
    assert progress["current_horizon"]["id"] == "product-weakness-queue-closed"
    ...
    assert progress["next_leaf"] == "none_product_weakness_queue_closed"
...
def test_progress_map_markdown_is_public_safe_and_decision_oriented() -> None:
    ...
    assert "product-weakness-queue-closed" in rendered
    assert "none_product_weakness_queue_closed" in rendered
