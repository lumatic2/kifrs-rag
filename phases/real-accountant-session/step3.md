...
2. actual capture manifest가 true인지 확인한다.
3. generated queue records가 public-safe이고 비어 있지 않은지 확인한다.
4. session manifest builder가 `actual_feedback` manifest를 생성하는지 확인한다.
5. RS3을 completed로 업데이트한다.
