...
## Current Horizon — portfolio-viz-site

<!-- harness:goal id="portfolio-viz-site" status="active" -->
...
<!-- harness:milestone id="PV1" status="completed" priority="P0" evidence="changesets/20260712-pv1-web-data-export/README.md; changesets/20260712-pv1-crossref-extraction/README.md" -->
### PV1 — 데이터 레이어 (export + 참조 추출)
- DoD: export JSON 세트 + public-safe gate 양방향(PASS/synthetic FAIL) + `cross_reference` 첫 적재(알려진 참조 존재 확인) + 검색 eval 비퇴행.
- Evidence: changesets/20260712-pv1-web-data-export/README.md; changesets/20260712-pv1-crossref-extraction/README.md
- Gap: 사이트에 넣을 데이터가 없음 — cross_reference 는 스키마만.
- Status: [x]

- Completed at: 2026-07-12
- Summary: 메타데이터 export 4종 + 전수 본문 대조 gate 양방향 + cross_reference 첫 적재(1,850행/655쌍), eval 비퇴행
<!-- harness:milestone id="PV2" status="completed" priority="P0" evidence="changesets/20260712-pv2-astro-scaffold-map/README.md; changesets/20260712-pv2-refgraph-island/README.md; changesets/20260712-pv2-evaldash-pipeline/README.md" -->
### PV2 — Astro 사이트 구현 (4뷰)
- DoD: `npm run build` 성공 + /browse 실브라우저 4뷰 E2E.
...
- Gap: 데이터만으론 포트폴리오가 아님 — 보여주는 표면 필요.
- Status: [x]

- Completed at: 2026-07-12
- Summary: Astro 4뷰 완성 — 서가 히어로·참조 그래프(d3-force)·eval 대시보드(validator PASS)·파이프라인, /browse E2E
<!-- harness:milestone id="PV3" status="active" priority="P1" -->
### PV3 — 배포 + 공개 검증
- DoD: `kifrs.askewly.com` 라이브 4뷰 렌더 + 배포 산출물 gate 재실행 PASS.
- Evidence: 라이브 URL + gate 로그
