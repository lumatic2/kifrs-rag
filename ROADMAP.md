...
**Phase 1 다음 체크박스**: `kifrs/parse.py` 작성 — PDF → 조·항·호 계층 JSON 파서.

- tax-agent `parse_exam_papers.py` 구조 포팅 (extract → split → dump JSON), 정규식은 위 "PDF 구조 샘플링" 항목의 패턴으로 재작성
- 먼저 1115호 단일 PDF 대상으로 PoC, 파싱 결과 수동 검증 후 나머지 4개 기준서로 확장
- 페이지 헤더/푸터 `- \d+ -` 제거, 문단 번호 줄 + 바디 결합, 한NN.N과 일반 문단 병합 처리 주의
- 산출 스키마(예시): `{standard: "1115", paragraphs: [{no: "5", body: "...", appendix: null, ko_added: false, page: 8}, ...]}`
