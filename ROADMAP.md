...
| **Issue-back 루프 (현재)** | ai-accounting-firm 실소비 결함 수리 — 1회전(H4) 완료 | ✅ 2026-07-12 |

## Current Horizon — h4-issue-back-repair closed

<!-- harness:goal id="h4-issue-back-repair" status="completed" -->
...
<!-- harness:milestone id="IB1" status="completed" priority="P0" evidence="docs/reports/2026-07-12-ib1-repricing-search-repair.md" -->
### IB1 — 리픽싱 계열 검색 수리
- DoD: 필터 없는 기본 경로(hybrid·reranked)에서 리픽싱 계열 쿼리가 1001-한138.5 계열에 도달 + standard 필터 함정 안내가 tool/skill 표면에 존재.
- Evidence: docs/reports/2026-07-12-ib1-repricing-search-repair.md
- Gap: H4 실소비에서 유일한 사실상 실패(mcp-log #7/#11). f-acc-task-01 ax_possible 승격 조건.
- Status: [x]

- Completed at: 2026-07-12
- Summary: 리픽싱/고정대고정 term_bridge 3건 + 필터 함정 안내 2표면 — 재현 쿼리 기본 경로 도달
<!-- harness:milestone id="IB2" status="completed" priority="P1" evidence="docs/reports/2026-07-12-ib2-parse-repair.md" -->
### IB2 — 파싱 수리 (BC 세분화 + 섹션 제목)
- DoD: 1001 한BC 문단 개별 조회 가능 + `list_sections` 제목 무결 + 재인제스트 후 문단 수·retrieval eval 비퇴행.
- Evidence: docs/reports/2026-07-12-ib2-parse-repair.md
- Gap: BC 통짜 문단 토큰 낭비(mcp-log #15) + 섹션 제목 잘림(mcp-log #1/#2).
- Status: [x]

- Completed at: 2026-07-12
- Summary: BC/DO/IN 9,598문단 세분화 + 섹션 제목 수리 + BC 감점으로 eval 전 지표 개선
<!-- harness:milestone id="IB3" status="completed" priority="P1" evidence="docs/reports/2026-07-12-h4-issue-back-repair-close-report.md" -->
...
- Completed at: 2026-07-12
- Summary: mcp-log 실패/부분 5사례 전부 해소 재검증 — horizon close
## Next Candidates
...
| 테이블 | 현재 | 시점 |
|---|---|---|
| `standard` / `paragraph` / `paragraph_fts` | ✅ 100개 / 17,896행 (BC/DO/IN 9,598 포함, 2026-07-12) / trigram | Phase 1·2 + IB2 |
| `embedding` | ✅ bge-m3 1024d, 17,896 (100%) | Phase 2 + IB2 |
