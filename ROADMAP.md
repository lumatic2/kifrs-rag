...
| `paragraph` | ✅ 8,328행 (2026-04-24 +1,339: letter-suffix 616건 + 파서 개선 동반 증가) | Phase 1·2 완료 |
...
- [x] **[CRITICAL 1]** `parse.py:RE_PARA_INLINE/ONLY/KO_PARA/PRAC_PARA` suffix 확장 `[A-Z]?` (2026-04-24) → 1109-4.1.2A 포함 letter-suffix **616건 복구**. 전체 8,328 paragraphs
- [x] goldset **Q006~Q008 cross-standard 함정 3문항** 작성 + DB 검증 통과 (2026-04-24)
- [x] **pytest + anthropic 인프라** — `pyproject.toml [project.optional-dependencies] eval` (2026-04-24)
- [x] **평가 하네스 구현** `kifrs/eval/` (2026-04-24)
  - `models.py`: GoldItem/RunResult/ScoreResult/ItemReport/RunReport 데이터클래스
  - `runners.py`: KifrsMcpRunner(store 직접 + Claude API cache) / BaselineRunner / NotebookLmManualRunner. Topic mapper + budget 균등 분배 retrieval
  - `scorers.py`: CiteScorer(precision/recall F1 + may 가점) / KeywordScorer(히트율 − forbidden 감점) / GlobalRulesScorer(DB miss 명시 로깅 — CRITICAL 2)
  - `reporter.py`: stdlib `string.Template` HTML + Markdown + JSON raw
  - `harness.py`: CLI 오케스트레이션
  - `tests/test_scorers.py`: 11 unit tests ✅
- [x] **데모 리포트** `data/eval/results/demo_20260424.html` (2026-04-24) — mock answer 기반 pipeline 검증. Composite 평균 **0.958** (Q001~Q008 전부 실행 + HTML·MD·JSON 3종 생성)

**Phase 2 종료 조건 잔여**:

- [ ] **`ANTHROPIC_API_KEY` 세팅 후** `kifrs-mcp` vs `baseline-noretrieval` 실측 비교 → B축 20%p 우위 검증 (현재 retrieval dry-run must_cite recall 33% — 하이브리드 검색 구현 후 개선 목표)
- [ ] **B축 평가셋 50문항 확장**: CPA 1·2차 기출 30 + 합성 20 (현재 goldset 8문항)
- [ ] **임베딩 인덱스** `kifrs/embed.py` — sentence-transformers 한국어 또는 text-embedding-3-small. 하이브리드 검색으로 retrieval recall 개선
- [ ] **크로스레퍼런스 추출** `kifrs/xref.py` — `cross_reference` 테이블 채우기
- [ ] UX 개선: `section_like` 부분 매칭, `get_paragraph_range`

**보류**: 임베딩·xref는 평가 하네스 기반 실측 후 우선순위 재조정.
