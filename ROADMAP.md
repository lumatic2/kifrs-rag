...
**Phase 2 설계 확정** (2026-04-17 gstack /office-hours + /plan-eng-review 세션)
- 방향: **평가 하네스 = 포트폴리오 데모 트랙** 통합. Approach B 선정
- 구체: "CPA 2차 서술형 3문항 자동풀이 + 자동 인용검증 리포트" 데모 → Phase 2 평가 하네스와 같은 코드베이스 공유
- 설계문서: `~/.gstack/projects/lumatic2-kifrs-rag/lumatic2-master-design-20260417-105951.md` (176줄)
- Eng review 결과: 2 critical gaps 도출
  - **CRITICAL 1**: `kifrs/parse.py:RE_PARA_INLINE` 을 `\d+(?:\.\d+)*[A-Z]?` 로 확장 (1109.4.1.2A suffix) → scorer와 공유
  - **CRITICAL 2**: Scorer DB miss 시 silent 0점 방지 — 명시 로깅 + 리포트 "DB miss" 섹션
- 아키텍처 확정: eval 모듈 8 file (models/runners/3·scorers/3·reporter)
  - Scorer 5→3 통합 (cite / keyword / global_rules)
  - HTML 템플릿은 stdlib `string.Template` (Jinja2 의존성 불필요)
  - Runner 직접 store 호출 (MCP spawn 피함)
  - Claude API 응답 cache 도입

**다음 작업 순서** (The Assignment → 구현):

1. **[CRITICAL 선행]** `kifrs/parse.py:RE_PARA_INLINE` suffix 확장 (`[A-Z]?`). 기존 파싱 회귀 없음 확인
2. **[The Assignment]** `data/eval/goldset.json`에 **Q006~Q008 cross-standard 함정 3문항 초안** 작성
   - Q006: 1115 ↔ 1109 (수익 거래가격 차감 vs 금융부채)
   - Q007: 1115 ↔ 1116 (변동 임차료 매출연동 vs 수익)
   - Q008: 1109 ↔ 1116 (리스부채 vs 금융부채 범위 제외)
   - 각 문항: must_cite 2-3 / keywords 3-5 / forbidden_keywords 2-3
3. **pytest 인프라 도입** — `pyproject.toml [project.optional-dependencies] eval = ["pytest>=8.0", "anthropic>=0.39.0"]`
4. **평가 하네스 구현** `kifrs/eval/` — models/runners/scorers/reporter
5. **데모 리포트 생성** → `data/eval/results/demo_YYYYMMDD.html`

**보류된 Phase 2 항목** (데모 트랙과 독립):
- UX 개선 — `section_like`, `get_paragraph_range`
- 크로스레퍼런스 추출 `kifrs/xref.py`
- 임베딩 인덱스 `kifrs/embed.py` (하이브리드 검색)

추천 순서: **1(파서 suffix) → 2(goldset Q006~Q008) → 3(pytest) → 4(eval 구현) → 5(데모 리포트)**. B축 평가셋 45문항 확장은 4 완료 후 병행
