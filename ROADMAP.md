...
## Current Horizon — drift-watch-automation

<!-- harness:goal id="drift-watch-automation" status="active" -->
...
<!-- harness:milestone id="DR3" status="active" priority="P0" -->
### DR3 — 주간 감지 + 세션 자동 경고
- DoD: 스케줄 task 등록+1회 실행 관측(로그·PENDING.json) + MCP 경고 발화/미발화 양방향 검증 + 기존 tool smoke 비퇴행.
- Evidence: schtasks 등록·실행 로그 + changeset checklist
