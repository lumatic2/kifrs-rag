...
> K-IFRS 기준서 + AI 도구체인으로 회계사 실무의 상당 부분을 본인이 수행하는 개인용 시스템. 본인 PC 로컬 비공개 운영.
> 완료 이력(Phase 1~4 + M1·M2·M3) → **`BACKLOG.md`** · 다음 세션 진입점 → **`CLAUDE.local.md`**

## 배경 / 포지셔닝

K-IFRS 기준서를 프로그램적으로 조회할 공식 API/MCP 부재. 빅4 사내 AI는 비공개, 외부 공개는 KASB·IFRS Foundation 저작권 장벽. **저작권법 제30조 사적이용·제35조의5 공정이용** 범위 안에서 본인 학습·실무·포트폴리오용 비공개 운영.
- 본인 PC 로컬 전용. 외부 공개·배포·공유 없음. 코드는 GitHub private
- `tax-agent` 패턴 재사용 → 신규 개발 비용 최소화. 외부 공개는 별도 후속 프로젝트로 분리
...
### Active Milestones
- (없음 — M1·M2·M3 완료. 아래 Next Candidates 에서 다음 milestone 승격)

### Next Candidates
- **M4 — Hierarchical retrieval + evidence curation** — 조·항·호 섹션→문단 2단계 검색 + 저신뢰 임계값 필터. M1 하네스로 before/after 측정. 잔여 miss 9건(Q001·Q004·Q006·Q008·Q029·Q039·Q040·Q041·Q048) 중 계층검색 후보: Q008(1109-2.1)·Q039(1037-14)
...
- 코드 레포는 GitHub private. 동료·친구 공유 ❌ (사적이용 범위 깨짐)
