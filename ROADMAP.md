...
## Closed Horizon — portfolio-viz-polish (2026-07-12, `portfolio_viz_polished`)

<!-- harness:goal id="portfolio-viz-polish" status="completed" -->
목표: kifrs.askewly.com 3축 polish — ① imagegen 비주얼 보강(서가/한지/인주 컨셉) ② 참조 그래프 인터랙션 심화 ③ 콘텐츠 확장(drift 스토리·수리 루프 연대기·상호 링크). 본문 비노출 gate 유지. 사용자 발제 예외 4건째(2026-07-12). (상세 → `plans/horizons/portfolio-viz-polish.md`, step 트리 → `archive/plans/2026-07-12-portfolio-viz-polish.md`)

<!-- harness:milestone id="PP1" status="completed" priority="P0" evidence="changesets/20260712-pp1-visuals/README.md — 6장 생성·반입, 실브라우저 5도판 로드, gate PASS" -->
### PP1 — 비주얼 보강 (imagegen)
...
- Evidence: changesets/20260712-pp1-visuals/README.md — 6장 생성·반입, 실브라우저 5도판 로드, gate PASS
<!-- harness:milestone id="PP2" status="completed" priority="P1" evidence="changesets/20260712-pp2-universe changesets/20260712-pp2-content changesets/20260712-pp2-deploy — 라이브 재검증" -->
### PP2 — 인터랙션 + 콘텐츠 + close
...
- Completed at: 2026-07-12
- Evidence: changesets/20260712-pp2-universe changesets/20260712-pp2-content changesets/20260712-pp2-deploy — 라이브 재검증
...
목표: H10 issue-back #4 수리 — search() 에 `section` 필터 추가(공시 절 조회 정밀도 구조 결함) + 절 단위 수집 정본 경로 표면화. 정규 issue-back 입력(2회전째). (상세 → `plans/horizons/h10-disclosure-search-repair.md`, step 트리 → `archive/plans/2026-07-12-h10-disclosure-search-repair.md`)

## Closed Horizon — portfolio-viz-site

<!-- harness:goal id="portfolio-viz-site" status="completed" -->
목표: 메타데이터 전용 포트폴리오 시각화 사이트 — 기준서 지도·참조 그래프·eval 대시보드·아키텍처 스토리 4뷰, Astro + Cloudflare(`kifrs.askewly.com`). 본문 비노출 public-safe gate 필수. 사용자 발제 예외 3건째(2026-07-12). (상세 → `plans/horizons/portfolio-viz-site.md`, step 트리 → `archive/plans/2026-07-12-portfolio-viz-site.md`)
...
목표: drift 감지의 무인화 — Windows 작업 스케줄러 주 1회 감지 + MCP 응답에 pending drift 자동 경고 (갱신은 수동 유지). 사용자 발제 예외 2건째(2026-07-12). (상세 → `plans/horizons/drift-watch-automation.md`, step 트리 → `archive/plans/2026-07-12-drift-watch-automation.md`)
...
목표: KASB 제·개정 공표와 로컬 DB 사이 drift 감지(MCP tool `check_drift` + `kifrs/drift.py` 코어) + 감지된 기준서 단위 갱신 경로(재다운로드→재인제스트→amendment 기록). 자체 발제 — 사용자 승인 예외(2026-07-12). (상세 plan → `plans/horizons/kasb-drift-watch.md`, step 트리 → `archive/plans/2026-07-12-kasb-drift-watch.md`)
...
- 없음 — 새 후보는 ai-accounting-firm 실소비 issue-back(`docs/BACKLOG.md` Issue-back Queue) 또는 사용자 승인 예외에서만 발생.

## Paused Horizons

<!-- harness:goal id="rag-optimization-resume" status="paused" --> `plans/horizons/rag-optimization-resume.md` — RO2 DoD 미확정.
<!-- harness:goal id="rag-agent-integration" status="paused" --> `plans/horizons/rag-agent-integration.md` — RGA2/RGA3 DoD 미확정.
...
최근 완료 horizon 상세는 `docs/BACKLOG.md` 참조 (2026-07-04~07-06: RAG quality, multi-authority, private parser, firm-facing surface, trust evidence, demo/rehearsal 계열 등 전체 완료).
