...
### 검색 품질 개선 (Round 1 신호 기반 trigger)
- [ ] **🔴 [1순위] 임베딩 인덱스** `kifrs/embed.py` — sentence-transformers 한국어 또는 text-embedding-3-small. 동의어 검색 실패(환매↔재매입, 내부창출 브랜드, 선택형 주식기준보상, 공매도) 해결
- [ ] **하이브리드 검색** — keyword + semantic 점수 결합. MCP tool `search_hybrid` 추가
- [ ] **🟡 [2순위] /accounting 시험 풀이 모드 분기** — 다단 sub-question + 계산 표 + 분개 코드블록 포맷 보강
- [ ] UX: `section_like` 부분 매칭, `get_paragraph_range`

### Dogfood 라운드 2 (개선 후 재검증)
- [ ] 같은 5문항 재실행 → recall 개선 측정 (60% → 70%+ 목표)
...
**Phase 2 진행 중** — Dogfood Round 1 종료, 검색 품질 개선 단계 진입.

1. [x] CLAUDE.md / ROADMAP.md 야망 격상 (2026-04-27)
2. [x] 회계사 2차 기출 5문항 수집 (2024 제59회 재무회계) → `data/dogfood/cpa2/`
3. [x] 5문항 dogfood Round 1 + 종합 분석 (2026-04-27)
4. [ ] **🔴 [1순위 진행 중] 임베딩 인덱스 `kifrs/embed.py`** — 모델 선택·인덱싱·하이브리드 검색
5. [ ] [2순위] /accounting 시험 풀이 모드 분기
6. [ ] [3순위] 학원 모범답안 입수 + Round 2 재실행
