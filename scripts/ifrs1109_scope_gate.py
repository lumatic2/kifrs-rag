...
BASELINE_RETRIEVER = "ifrs1115_subquery_hybrid"
TARGET_RETRIEVER = "ifrs1109_scope_hybrid"
PRESERVED_ITEMS = ("Q001", "Q004", "Q006", "Q013", "Q025", "Q026", "Q029", "Q041")
RECOVERED_SCOPE_ITEMS = ("Q008",)
REJECTED_ITEMS = ("Q040",)
DEFAULT_K = 20


def build_report(*, goldset: Path = DEFAULT_GOLDSET, k: int = DEFAULT_K) -> dict[str, Any]:
    wanted = set(PRESERVED_ITEMS + RECOVERED_SCOPE_ITEMS + REJECTED_ITEMS)
...
def evaluate_gate(report: dict[str, Any]) -> dict[str, Any]:
    ...
    recovered_scope = _group_rows(RECOVERED_SCOPE_ITEMS, baseline_items, target_items)
    ...
    for item_id, row in recovered_scope.items():
        if row["baseline_miss"] and not row["target_miss"]:
            continue
        failures.append(
            f"{item_id} 1109 scope subquery did not recover expected miss: "
...
def render_text(payload: dict[str, Any]) -> str:
    lines = [
        ...
        f"ifrs1109_scope_recall@20: {payload['target_recall20']:.3f}",
    ]
    for label in ("preserved", "recovered_scope", "rejected"):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Gate opt-in IFRS 1109 scope-exclusion retriever.")
