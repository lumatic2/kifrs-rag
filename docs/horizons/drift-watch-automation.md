...
`drift_watch_automated` (2026-07-12) — changeset 2개(#38/#39). 주간 task `kifrs-drift-weekly`
(월 09:00, StartWhenAvailable) 등록·1회 실행 관측(LastTaskResult 0), PENDING.json 상태 유지,
MCP search/get_paragraph 가 pending drift 기준서에 `drift_warning` 자동 첨부(발화/미발화 양방향
검증). 재등록 경로 `scripts/register_drift_task.ps1` 레포 보존.

## Objective 임팩트

최신성 유지가 "사람이 물어야 도는" 도구에서 **무인 루프**(주간 감지 → pending 상태 → 세션 내
자동 경고)로 전환. 남은 수동 지점은 갱신 실행뿐이며 이는 사용자 결정(eval 비퇴행 확인 기회
보존)이다. 다음 horizon 은 여전히 issue-back 또는 사용자 승인 예외가 연다.
