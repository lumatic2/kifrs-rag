...
- [ ] PV1-S1: `scripts/export_web_data.py` — DB→`web/src/data/*.json`(기준서 계층·문단 규모·eval 이력·파이프라인 통계) + public-safe gate 스크립트 (verify: export 실행 JSON 생성 + gate PASS + synthetic 본문 주입 시 gate FAIL)
