...
- [x] DS1-S1: search tool `section`/`exclude_bc` 필터 + docstring (verify 3/3: 필터 결과 전부 해당 절 + 미지정 시 기존 동작 바이트 동일 + 미존재 section 빈 결과). **편차**: store/embed 관통 대신 MCP 레이어 후처리 필터 + 오버샘플(×5, 100~500) — 5개 모드 한 곳 커버, eval 경로 불변으로 비퇴행 구조 보장 (changeset #46 `20260712-ib4-search-section-filter` Contract 참조 — 워커 구현·검증본이 정본, 5개 기준서 전수 before/after 표 포함)
