...
현재 active horizon 없음. 방금 완료한 horizon은 `real-anonymized-transaction-poc`이다.
...
다음 추천 horizon은 `accountant-feedback-incorporation`이다. 목표는 실제 회계사 피드백을 demo brief,
review questions, eval/backlog rules에 반영하는 것이다.
