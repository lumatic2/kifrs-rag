...
- packaging remains excluded until the user explicitly asks for it
...
| 2 | `private-parser-realism-hardening` | Local parser evidence is still fixture-heavy; actual local-file adapter evidence remains gated by explicit authorization. | The objective needs realistic client-input handling, but protected payloads must stay local and authorization-gated. | active | PPR1 authorization-safe adapter proof plan | `docs/reports/2026-07-05-ppr1-authorization-safe-adapter-proof.md` |
| 3 | `external-source-body-connector-expansion` | External sources have metadata/demo evidence, but broader source-body connector implementation is still missing. | Accounting work needs regulator, interpretive, legal, filing, and policy evidence beyond K-IFRS paragraphs. | pending | ESB1 source-body connector selection and policy gate | `docs/reports/2026-07-05-esb1-source-body-connector-selection.md` |
...
- Active horizon: `private-parser-realism-hardening`
- Run horizons in order unless a later gap becomes objectively blocking.
- Do not add packaging or external feedback work to this queue without explicit user instruction.
...
```json
{
  ...
  "active_horizon": "private-parser-realism-hardening",
  "candidates": [
    {
      "order": 1,
