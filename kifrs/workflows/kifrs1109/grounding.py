"""런타임 citation 존재 검증 (RGA1).

결정 엔진의 reasons 문자열에 하드코딩된 조항 인용(예: `[1109-4.1.2(b), B4.1.7~7B]`)이 실제 DB에
존재하는 조항을 가리키는지 검증한다. `kifrs.store`를 직접 import한다(MCP 프로토콜 경유 아님) —
엔진은 같은 프로세스 내부 코드이므로 프로세스 경계를 넘는 MCP 레이어가 필요 없다
...
def extract_citations(reason: str) -> list[str]:
    """reason 문자열의 `[1109-...]` 블록에서 `표준-조항` 토큰 리스트를 추출한다."""
    tokens: list[str] = []
...
def _base_paragraph_no(token: str) -> str:
    """`4.1.2(b)` -> `4.1.2`, `B4.1.7~7B` -> `B4.1.7` (범위 시작점만 조회 대상)."""
    no = token.split("-", 1)[1]
    no = no.split("~", 1)[0]
