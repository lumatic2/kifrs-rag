...
## 1109 classification hybrid 구현 (2026-07-05)

`ifrs1109_classification_hybrid` opt-in retriever를 추가했다. 기본 `hybrid`는 변경하지 않았다. Q040은
1109 분류 경계 문제로, SPPI 불충족 표현 자체보다 "상각후원가/FVOCI 조건이 아니면 FVTPL" 잔여 분류
규칙을 보조 쿼리로 넣을 때 회복된다.

`docs/reports/2026-07-05-ifrs1109-classification-hybrid-implementation.md` 기준 full 50-item recall@20은
0.980에서 1.000으로 올랐고, citation-level absent는 1개에서 0개가 됐다. `scripts/ifrs1109_classification_gate.py`
는 기존 회복 항목 보존과 Q040 회복을 함께 검증한다. 단, 이 retriever는 reviewed rule-triggered repair
layer의 누적이므로 기본 retriever로 바로 승격하지 않고, 통합 RAG quality close report와 실사용 검증을
다음 단계로 둔다.

## 통합 close report (2026-07-05)

`docs/reports/2026-07-05-rag-quality-refresh-close-report.md`와 `scripts/rag_quality_final_gate.py`를 추가했다.
최종 opt-in retriever `ifrs1109_classification_hybrid` 기준 50-item retrieval-only recall@20은 1.000이고
citation-level absent는 0이다. 기본 `hybrid`는 그대로 유지한다.

다음 결정: 이 repair-layer stack을 기본으로 승격하지 말고, real-accountant-session demo에서 opt-in으로
사용해 실제 질문에서 과적합/오탐이 없는지 확인한다. 이후 Accounting Intelligence Expansion Horizon 2
`Authority Source Map`으로 넘어간다.
